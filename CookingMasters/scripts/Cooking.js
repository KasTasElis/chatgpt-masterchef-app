...
export const Cooking = {
  ...
  updateTimer: (timer) => {
    timer.current--;
    timer.element.querySelector("p").textContent = `${String(
      Math.floor(timer.current / 60)
    ).padStart(2, "0")}:${String(timer.current % 60).padStart(2, "0")}`;
    if (timer.current < 20) {
  ...
  step: (index) => {
    ...
    document.querySelector("#cooking h3").textContent = `${index + 1} of ${
      Cooking.recipe.steps.length
    } | ${step.name}`;
    ...
    document.querySelector("#btn-next").textContent =
      index == Cooking.recipe.steps.length - 1 ? "You finished!" : "Next Step";
  },
};
